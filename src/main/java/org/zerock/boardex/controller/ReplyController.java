...
public class ReplyController {
    ...
    //@PathVariable("bno") : ?bno=197와 같은 쿼리 스트링의 값을 전달받음.
    //Long bno로 bno전달 받고, PageRequest로 댓글을 페이징함
    (@PathVariable("bno")Long bno, PageRequestDTO pageRequestDTO
    ){
        //bno번호에 해당하는 게시글의 댓글 목록을 페이징해서 resResponseDTO에 저장
    ...
    @GetMapping("/{rno}")
    //@PathVariable("rno") : ?rno=197와 같은 쿼리 스트링의 값을 전달받음.
    ...
    public Map<String, Long> modify(
        ...
        //resultMap 객체 생성
        Map<String, Long> resultMap=new HashMap<>();
        //resultMap에 댓글번호(rno)로 수정한 댓글 저장
        resultMap.put("rno", rno);
        //수정한 그 결과 반환해주샘.
        return resultMap;
    }

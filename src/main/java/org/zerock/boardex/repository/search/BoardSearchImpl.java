...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    @Override
    //String[] types:검색타입(제목,내용,작성자)
    //String keyword:검색창에 검색어 입력하는 키워드
    //Pageable:페이지
    public Page<BoardListAllDTO> searchWithAll(String[] types, String keyword, Pageable pageable){
        //Querydsl 이용한 동적 SQL의 처리
        QBoard board=QBoard.board;
        QReply reply=QReply.reply;
        //JPQLQuery 사용하려고 위에서 Q붙은애들 쓴것
        //JPQLQuery:자바언어에 가까운 질의어(쿼리)를 실행해달라는 뜻(게시판(board) 객체에).
        JPQLQuery<Board> boardJPQLQuery=from(board);
        
        //최종적으로 이 질의어를 쓰기위해서 위에 명령어들 쓴것
        //leftouterjoin: reply객체와 board객체를 조인하고 null값 허용
        boardJPQLQuery.leftJoin(reply).on(reply.board.eq(board));

        //검색 조건과 키워드 사용
        //types가 null이 아니고 길이가 0보다 크고, keyword값이 null이 아니라면
        if((types != null && types.length >0) && keyword != null){
            //where뒤의 조건을 쿼리로 만들어주는 객체
            BooleanBuilder booleanBuilder = new BooleanBuilder();
            //types개수만큼 반복(3개 반복)
            for(String type:types){
                switch (type){
                    case "t":
                        //글제목에 keyword가 포함되어있는 데이터 검색
                        //질의어 버전: where title='keyword'
                        booleanBuilder.or(board.title.contains(keyword));
                        break;
                    case "c":
                        //글내용에 keyword가 포함되어있는 데이터 검색
                        booleanBuilder.or(board.content.contains(keyword));
                        break;
                    case "w":
                        //글작성자에 keyword가 포함되어있는 데이터 검색
                        booleanBuilder.or(board.writer.contains(keyword));
                        break;
                }
            }
        ...
        //left outer join한 결과에 paging추가
        getQuerydsl().applyPagination(pageable, boardJPQLQuery);

        //Tuple(행):각각 다른 타입의 데이터를 추출할 때 사용
        //데이터를 조회해서 튜플형식으로 저장하겠다는 뜻.

...
public interface BoardService {
    ...
    //디폴트 메서드(default method)를 구현하여 그대로 자식 객체로 상속해 줄 수있음.
    default Board dtoToEntity(BoardDTO boardDTO){
        ...
        //getFileName : 첨부파일의 파일명이 null이 아니라면 = 첨부파일이 있으면
        if(boardDTO.getFileNames() != null){
            //첨부파일 개수만큼 반복
            boardDTO.getFileNames().forEach(fileName->{
                //첨부파일명을 _로 나누어서(split) arr배열에 저장
                //arr[0]=4f5763f6-b540-42f1-b4df-b6911437b6a9
                //arr[1]=dd.jpg
    ...
    default BoardDTO entityToDTO(Board board){
        ...
        List<String> fileNames=board.getImageSet()
                //매핑(map):
                .stream().sorted().map(boardImage ->
                    boardImage.getUuid()+"_"+boardImage.getFileName()
                //collect: 앞에 자료들 합쳐달라는의미 list형이므로 tolist 사용

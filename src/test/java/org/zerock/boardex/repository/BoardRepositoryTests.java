...
public class BoardRepositoryTests {
    ...
    public void testInsertWithImages(){
        Board board=Board.builder()
                ...
                .build();
        for(int i=0; i<3; i++){
            //UUID.randomUUID().toString(),"file"+i+".jpg : 파일 이름 정해주는 메서드
    ...
    public void testInsertAll(){
        ...
        for(int i=1; i<=100; i++){
            ...
            for(int j=0; j<3; j++){
                ...
                //continue:반복문 빠져나감, i값을 5로 나눈 나머지가 0이면 for문 빠져나감
                if(i % 5 == 0){
                    continue;
                }
    ...
    public void testSearchImageReplyCount(){
        //페이징(현재 페이지0번, 페이지크기10으로 페이징)한 객체를 게시글 번호로 내림차순하여 pageable에 저장
        Pageable pageable=PageRequest.of(0, 10, Sort.by("bno").descending());
